...
def preprocess_clevr_test(features, resolution, apply_crop=False,
  ...
  for i in range(len(mask)):
      cur_mask = mask[i]
      cur_mask = tf.cast(cur_mask, dtype=tf.float32)
      cur_mask = (cur_mask / 255.0) # Rescale to [0,1] 

      if apply_crop:
        crop = ((29, 221), (64, 256))  # Get center crop.
        cur_mask = cur_mask[crop[0][0]:crop[0][1], crop[1][0]:crop[1][1], :]

      cur_mask = tf.image.resize(cur_mask, resolution, method=tf.image.ResizeMethod.BILINEAR)
      cur_mask = tf.clip_by_value(cur_mask, 0., 1.) 
      cur_mask = cur_mask > 0
      masks.append(cur_mask)

  masks = tf.dtypes.cast(masks, tf.int32)
  p_mask = tf.zeros((128,128),tf.int32)
  p_mask = tf.math.argmax(masks,0)
 
  features = {"image": image, "mask": p_mask}
  return features

def build_clevr_test(resolution=(128, 128), max_n_objects=6, apply_crop=False):

  tf_records_path = 'clevr_with_masks_train.tfrecords'
  ds = clevr.dataset(tf_records_path)

  def filter_fn(example, max_n_objects=max_n_objects):
    return tf.less_equal(tf.math.count_nonzero(example['material']),
                         tf.constant(max_n_objects, dtype=tf.int64))

  ds = ds.filter(filter_fn)

  def _preprocess_fn(x, resolution, max_n_objects=max_n_objects):
    return preprocess_clevr_test(
        x, resolution, apply_crop=apply_crop, get_properties=False,
...
def build_test_iterator(batch_size, **kwargs):
  ds = build_clevr_test(**kwargs)
  ds = ds.repeat(-1)
...
def build_clevr_iterator(batch_size, split, **kwargs):
  ...
  #build_test_iterator(batch_size = 1, apply_crop=True,max_n_objects=4)